...
import { Users, Activity, CheckCircle, Droplet, Clock3, Target } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-on-surface-variant font-medium mt-1">System-wide metrics and verification overview.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="flex flex-col border-b-4 border-b-primary shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-4xl font-black text-on-surface tracking-tighter">{Number(data?.users || 0).toLocaleString()}</p>
        ...
        <Card className="flex flex-col shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-4xl font-black text-on-surface tracking-tighter">{Number(data?.hospitals || 0).toLocaleString()}</p>
        ...
        <Card className="flex flex-col shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-4xl font-black text-on-surface tracking-tighter">{Number(data?.bloodBanks || 0).toLocaleString()}</p>
        ...
        <Card className="flex flex-col border-b-4 border-b-red-500 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-4xl font-black text-on-surface tracking-tighter">{Number(data?.bloodRequests || 0).toLocaleString()}</p>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 bg-surface-container-low p-6 rounded-2xl">
            <div className={`bg-white p-4 rounded-xl text-center border-b-2 border-orange-400 shadow-sm`}>
              ...
              <p className={`text-2xl font-bold text-orange-600`}>{Number(data?.pendingHospitals || 0).toLocaleString()}</p>
            </div>
             <div className={`bg-white p-4 rounded-xl text-center border-b-2 border-orange-400 shadow-sm`}>
              <p className={`text-[10px] font-bold mb-1 text-orange-500`}>Blood Banks</p>
              <p className={`text-2xl font-bold text-orange-600`}>{Number(data?.pendingBloodBanks || 0).toLocaleString()}</p>
            </div>
          </div>
        </div>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 bg-surface-container-low p-6 rounded-2xl">
            <div className="bg-white p-4 rounded-xl border-b-2 border-blue-400 shadow-sm">
              ...
              <p className="text-2xl font-bold text-blue-700 mt-2">
                {Number(data?.requestsToday || 0).toLocaleString()}
              </p>
            </div>
            <div className="bg-white p-4 rounded-xl border-b-2 border-emerald-400 shadow-sm">
              ...
              <p className="text-2xl font-bold text-emerald-700 mt-2">
                {Number.isFinite(Number(data?.fulfillmentRate))
                  ? `${Number(data.fulfillmentRate).toFixed(1)}%`
                  : '0.0%'}
              </p>
            </div>
          </div>
      ...
      <div className="space-y-4">
        ...
        <Card className="p-0 overflow-hidden">
          <div className="p-4 text-center text-slate-500 text-sm">Audit activity feed will be connected in a later chunk.</div>
        </Card>
      </div>
    </div>
  );
